...
describe 'recipe::app_test::remote' do

  it "server is created" do
    file("#{node[:wlp][:base_dir]}/wlp/usr/servers/jsp-examples-remote/server.xml").must_exist
  end

  it "boots on startup" do
    service("wlp-jsp-examples-remote").must_be_enabled
  end

  it "runs as a daemon" do
    # Work-around as described in https://github.com/calavera/minitest-chef-handler/issues/22
    s = service("wlp-jsp-examples-remote")
    s.supports([:status])
  ...
  def send_request
    for i in 1..10
      begin
        url = URI.parse('http://localhost:9080/jsp-examples-remote/jsp2/el/basic-arithmetic.jsp')
